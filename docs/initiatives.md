...
- [ ] **`verify_upgrade` MCP tool** — runs the project's test suite post-codemod, parses failures, maps them back to migration issues; closes the agent loop (analyze → apply → verify)
